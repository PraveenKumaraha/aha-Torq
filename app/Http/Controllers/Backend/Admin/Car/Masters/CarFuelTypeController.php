...
class CarFuelTypeController extends Controller
{
   ...
   public function index()
   {
      ...
      return view('Car.Masters.FuelType.index');
   }
   public function getAllCarFuelTypes(Request $request)
   {

      if ($request->ajax()) {

         $queries = CarFuelType::orderby('created_at', 'desc')->get();
   ...
   public function create(Request $request)
   {
      if ($request->ajax()) {
         ...
         $view = View::make('Car.Masters.FuelType.create')->render();
   ...
   public function edit(Request $request, CarFuelType $carFuelType)
   {
      if ($request->ajax()) {


         $view = View::make('Car.Masters.FuelType.edit', compact('carFuelType'))->render();
         return response()->json(['html' => $view]);
      } else {
         return response()->json(['status' => 'false', 'message' => "Access only ajax request"]);
      }
   ...
   public function update(Request $request, CarFuelType $carFuelType)
   {
      ...
      if ($request->ajax()) {
         ...
         if ($validator->fails()) {
            ...
            $model = CarFuelType::findOrFail($carFuelType->id);
            $model->name = $request->input('name');
   ...
   public function destroy(Request $request, CarFuelType $carFuelType)
   {
      if ($request->ajax()) {
